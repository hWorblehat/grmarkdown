...
public class MarkdownPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		pluginManager.apply(MarkdownBasePlugin)
		pluginManager.apply(LifecycleBasePlugin)
	}
	...
	static class Rules extends RuleSource {
		...
		void addMarkdownCompileTask(ModelMap<Task> tasks, @Path('buildDir') final File buildDir) {
			tasks.create(MARKDOWN_TASK_NAME, MarkdownCompile) { MarkdownCompile task ->
				task.srcDir(DEFAULT_SRC_DIR)
				task.setDestinationDir(new File(buildDir, DEFAULT_DESTINATION_DIR))
				task.source.setName(DEFAULT_SRC_NAME)
			}
			
			tasks.named(LifecycleBasePlugin.ASSEMBLE_TASK_NAME) { Task assemble ->
